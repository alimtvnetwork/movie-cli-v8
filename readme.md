...
Installs exactly the version in the URL — never auto-upgrades. Use this for CI pipelines, Dockerfiles, reproducible setups, or when you need to roll back. Replace `v2.324.0` with the [release tag](https://github.com/alimtvnetwork/movie-cli-v8/releases) you want.

**Windows (PowerShell)**

```powershell
irm https://github.com/alimtvnetwork/movie-cli-v8/releases/download/v2.324.0/install.ps1 | iex
```

**Linux / macOS**

```bash
curl -fsSL https://github.com/alimtvnetwork/movie-cli-v8/releases/download/v2.324.0/install.sh | bash
```
...
The script attached to each release has the version baked in (`PINNED_VERSION="v2.324.0"`) and will install **exactly** that tag — it never falls back to "latest" and never delegates to the bootstrap scripts. Replace `v2.324.0` with any [published release](https://github.com/alimtvnetwork/movie-cli-v8/releases).
