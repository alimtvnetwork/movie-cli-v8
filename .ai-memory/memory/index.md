...
- [Boolean naming lint guard](mem://features/boolean-naming-lint-guard) — v2.307.0 CI guard scripts/check-boolean-naming.sh fails on (Is|Has)(Un|Not|No)+letter; allow-lists stdlib os.IsNotExist + safe English words (IsUnique, IsNoteworthy, HasNotes, IsUntitled, IsNormal); wired into ci.yml lint job
- [TV season ingestion](mem://features/tv-season-ingestion) — v2.308.0 wires Season/Episode tables into live scan: tmdb.GetTVSeason + cmd/movie_scan_tv_seasons.go::ingestTVSeasons triggered from linkScanMediaRelations when m.Type==TV; FK-safe via db.InsertSeason/InsertEpisode
- [movie tv command](mem://features/movie-tv-command) — v2.309.0 `movie tv [seasons|episodes|mark|unmark]` reads Season/Episode tables; renamed db.MarkEpisodeUnwatched→MarkEpisodePending; new db.FindEpisodeByMediaAndCode helper
- [Reverse-sync pass](mem://features/reverse-sync) — v2.310.0 movie scan reverse pass (DB→JSON→disk). Flags --no-reverse-sync, --reverse-sync-only. 4 new ReconAction* constants seeded by migrate_v6. Trust order: DB > JSON > TMDb. Spec 02-reverse-sync-spec.md; diagram 17-reverse-sync-flow.mmd
