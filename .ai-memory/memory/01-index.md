...
- [Movie CLI v8 Context, Topology & Memory Ingestion](learned/15-movie-cli-context-and-topology-ingestion.md) — Comprehensive onboarding, git commit history audit, lowercase readme auto-healing, SQLite schema contracts, CODE RED rules, and Antigravity skills scaffolding.
- [Movie CLI Codebase Deep Dive & Domain Skills](learned/16-movie-cli-codebase-deep-dive-and-domain-skills.md) — Comprehensive domain architecture breakdown and 7-part domain skills set (.agents/skills/) covering scanner, database, tmdb, file operations, rest api, updater/doctor, and testing.
- [GitMap CI/CD Runner Parity, Zero-Storage Workflows & Coding Guidelines Sync](learned/17-gitmap-cicd-runner-parity-and-guidelines-sync.md) — Local runner smart execution parity, Zero-Storage GitHub Actions workflow refactor, multi-repo sync safety, and canonical prompts/specs synchronization.
...
- [Release Architecture Map](02-release-architecture-map.md) — Architectural map of version propagation, sync pipeline (37-bump-version.py, 14-version-sync-checker.py), and 5-step release branching ceremony.
